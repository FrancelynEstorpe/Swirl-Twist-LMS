...
<script>
    $(document).ready(function() {
        ...
        function toInputDateTime(dtStr){
            if(!dtStr) return '';
        ...
        $('body').on('click', '.receive-booking', function() {
            ...
            $.get("{{ route('assignedBooking.index') }}" + '/' + booking_id + '/edit', function(data) {
                ...
                // enforce minimum as booking_schedule and default value accordingly
                const minSched = toInputDateTime(data.booking_schedule);
                if(minSched){
                    $('#pickup_schedule').attr('min', minSched);
                }
                const currentPickup = toInputDateTime(data.pickup_schedule) || minSched;
                if(currentPickup){
                    $('#pickup_schedule').val(currentPickup);
                }
